...
{
	public class DebugLogItem : ObjectController, IPointerClickHandler
	{
...
#pragma warning restore 0649
		...
		public void SetContent( DebugLogEntry logEntry, DebugLogEntryTimestamp? logEntryTimestamp, int entryIndex, bool isExpanded )
		{
			...
			if( isExpanded )
			{
				...
				if( !copyLogButton.gameObject.IsObjectActive() )
				{
					copyLogButton.gameObject.SetObjectActive( true );
			...
			else
			{
				...
				if( copyLogButton.gameObject.IsObjectActive() )
				{
					copyLogButton.gameObject.SetObjectActive( false );
		...
		public void ShowCount()
		{
			...
			if( !logCountParent.IsObjectActive() )
				logCountParent.SetObjectActive( true );
		}
		...
		{
			if( logCountParent.IsObjectActive() )
				logCountParent.SetObjectActive( false );
		}
