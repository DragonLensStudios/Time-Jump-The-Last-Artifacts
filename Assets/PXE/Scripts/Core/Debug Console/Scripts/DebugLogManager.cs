...
namespace PXE.Core.Debug_Console.Scripts
{
	...
	public class DebugLogManager : ObjectController
	{
...
#pragma warning disable 0649
		...
		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, console window will persist between scenes (i.e. not be destroyed when scene changes)" )]
		private bool singleton = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "Minimum height of the console window" )]
		private float minimumHeight = 200f;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, console window can be resized horizontally, as well" )]
		private bool enableHorizontalResizing = false;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, console window's resize button will be located at bottom-right corner. Otherwise, it will be located at bottom-left corner" )]
		private bool resizeFromRight = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "Minimum width of the console window" )]
		private float minimumWidth = 240f;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If disabled, no popup will be shown when the console window is hidden" )]
		private bool enablePopup = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, console will be initialized as a popup" )]
		private bool startInPopupMode = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, console window will initially be invisible" )]
		private bool startMinimized = false;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, pressing the Toggle Key will show/hide (i.e. toggle) the console window at runtime" )]
		...
		[SerializeField]
		[HideInInspector]
		public InputAction autocompleteBinding = new InputAction( "Autocomplete Binding", type: InputActionType.Button, binding: "<Keyboard>/tab", expectedControlType: "Button" );
#else
		[SerializeField]
		[HideInInspector]
		...
		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, the console window will have a searchbar" )]
		private bool enableSearchbar = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "Width of the canvas determines whether the searchbar will be located inside the menu bar or underneath the menu bar. This way, the menu bar doesn't get too crowded on narrow screens. This value determines the minimum width of the canvas for the searchbar to appear inside the menu bar" )]
		private float topSearchbarMinWidth = 360f;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, the console window will continue receiving logs in the background even if its GameObject is inactive. But the console window's GameObject needs to be activated at least once because its Awake function must be triggered for this to work" )]
		...
		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, the arrival times of logs will be recorded and displayed when a log is expanded" )]
		private bool captureLogTimestamps = false;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, timestamps will be displayed for logs even if they aren't expanded" )]
		internal bool alwaysDisplayTimestamps = false;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "While the console window is hidden, incoming logs will be queued but not immediately processed until the console window is opened (to avoid wasting CPU resources). When the log queue exceeds this limit, the first logs in the queue will be processed to enforce this limit. Processed logs won't increase RAM usage if they've been seen before (i.e. collapsible logs) but this is not the case for queued logs, so if a log is spammed every frame, it will fill the whole queue in an instant. Which is why there is a queue limit" )]
		private int queuedLogLimit = 256;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, the command input field at the bottom of the console window will automatically be cleared after entering a command" )]
		private bool clearCommandAfterExecution = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "Console keeps track of the previously entered commands. This value determines the capacity of the command history (you can scroll through the history via up and down arrow keys while the command input field is focused)" )]
		private int commandHistorySize = 15;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, while typing a command, all of the matching commands' signatures will be displayed in a popup" )]
		private bool showCommandSuggestions = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, on Android platform, logcat entries of the application will also be logged to the console with the prefix \"LOGCAT: \". This may come in handy especially if you want to access the native logs of your Android plugins (like Admob)" )]
...
#pragma warning disable 0169
		...
		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "Native logs will be filtered using these arguments. If left blank, all native logs of the application will be logged to the console. But if you want to e.g. see Admob's logs only, you can enter \"-s Ads\" (without quotes) here" )]
...
#pragma warning restore 0414
		...
		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, on Android and iOS devices with notch screens, the console window will be repositioned so that the cutout(s) don't obscure it" )]
		private bool avoidScreenCutout = true;

		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, on Android and iOS devices with notch screens, the console window's popup won't be obscured by the screen cutouts" )]
		internal bool popupAvoidsScreenCutout = false;

		[SerializeField]
		[field: Tooltip( "If a log is longer than this limit, it will be truncated. This helps avoid reaching Unity's 65000 vertex limit for UI canvases" )]
		private int maxLogLength = 10000;

#if UNITY_EDITOR || UNITY_STANDALONE || UNITY_WEBGL
		[SerializeField]
		[HideInInspector]
		[field: Tooltip( "If enabled, on standalone platforms, command input field will automatically be focused (start receiving keyboard input) after opening the console window" )]
		...
		[SerializeField]
		private ObjectController allButtons;
...
#pragma warning restore 0649
		...
		{
			get { return popupManager.gameObject.IsObjectActive(); }
			set { popupManager.gameObject.SetObjectActive( value ); }
		}
		...
		private string currentCommandText = ""; // To track the previous content of the TMP_InputField
		...
		public override void Start()
		{
			...
			{
				searchbar = null;
				searchbarSlotTop.gameObject.SetObjectActive( false );
				searchbarSlotBottom.gameObject.SetObjectActive( false );
			}

			if( commandSuggestionsContainer.gameObject.IsObjectActive() )
				commandSuggestionsContainer.gameObject.SetObjectActive( false );
			...
			commandInputField.onValueChanged.AddListener( OnCommandValueChanged );
			commandInputField.onSubmit.AddListener( OnEndEditCommand );
			...
			autocompleteBinding.performed += (context) =>
			{
				if (string.IsNullOrEmpty(currentCommandText)) return;

				if (string.IsNullOrEmpty(commandInputFieldAutoCompleteBase) || !commandInputFieldAutoCompleteBase.Equals(currentCommandText))
				{
					commandInputFieldAutoCompleteBase = currentCommandText;
				}

				string autoCompletedCommand = DebugLogConsole.GetAutoCompleteCommand(commandInputFieldAutoCompleteBase, currentCommandText);

				if (string.IsNullOrEmpty(autoCompletedCommand) || autoCompletedCommand == currentCommandText) return;
				...
				currentCommandText = autoCompletedCommand;

				// Reset autocomplete base for future autocompletions
				commandInputFieldAutoCompleteBase = string.Empty;

				RefreshCommandSuggestions(currentCommandText);
			};
		...
		private void OnValidate()
		{
			...
			if( UnityEditor.EditorApplication.isPlaying )
			{
				...
				// filterInfoButton.gameObject.SetObjectActive( receiveInfoLogs );
				// filterWarningButton.gameObject.SetObjectActive( receiveWarningLogs );
				// filterErrorButton.gameObject.SetObjectActive( receiveErrorLogs || receiveExceptionLogs );
			}
		}
		...
		IEnumerator SetCaretPositionNextFrame()
		{
			yield return null;
		...
		public override void Update()
		{
			base.Update();
#if !IDG_OMIT_ELAPSED_TIME
			lastElapsedSeconds = UnityEngine.Time.realtimeSinceStartup;
#endif
#if !IDG_OMIT_FRAMECOUNT
		...
		public override void LateUpdate()
		{
			base.LateUpdate();
#if UNITY_EDITOR
            if( isQuittingApplication || queuedLogEntries == null )
				return;
#endif

			int numberOfLogsToProcess;
			if (queuedLogEntries != null)
			{
				numberOfLogsToProcess = isLogWindowVisible ? queuedLogEntries.Count : ( queuedLogEntries.Count - queuedLogLimit );
			}
			else
			{
			...
			popupManager.SetLogCount(infoEntryCount, warningEntryCount, errorEntryCount);
			if( isLogWindowVisible )
			{
				...
				if( !Mathf.Approximately( logWindowWidth, logWindowPreviousWidth ) )
				{
					...
					if( searchbar )
					{
						if( logWindowWidth >= topSearchbarMinWidth )
						{
							...
							{
								searchbarSlotTop.gameObject.SetObjectActive( true );
						...
						else
						{
							...
							{
								searchbarSlotBottom.gameObject.SetObjectActive( true );
				...
				if( snapToBottom )
				{
					...
					if( snapToBottomButton.IsObjectActive() )
						snapToBottomButton.SetObjectActive( false );
				}
				else
				{
					float scrollPos = logItemsScrollRect.verticalNormalizedPosition;
					if( snapToBottomButton.IsObjectActive() != ( scrollPos > 1E-6f && scrollPos < 0.9999f ) )
						snapToBottomButton.SetObjectActive( !snapToBottomButton.IsObjectActive() );
				}
		...
		public void ShowLogWindow()
		{
			...
			logWindowTR.gameObject.SetObjectActive(true);
			allButtons.SetObjectActive(true);
		...
		public void HideLogWindow()
		{
			...
			if( OnLogWindowHidden != null )
				OnLogWindowHidden();
			logWindowTR.gameObject.SetObjectActive(false);
			allButtons.SetObjectActive(false);
		...
		private void RefreshCommandSuggestions( string command )
		{
			...
			{
				if (!commandSuggestionsContainer.gameObject.IsObjectActive())
					commandSuggestionsContainer.gameObject.SetObjectActive(true);
				...
				for (int i = 0; i < suggestionsCount; i++)
				{
					if (i >= visibleCommandSuggestionInstances)
					{
						if (i >= suggestionInstancesCount)
							...
							commandSuggestionInstances[i].gameObject.SetObjectActive(true);
					...
					if (caretArgumentIndex > 0)
						...
						sharedStringBuilder.Append(commandSuggestionHighlightStart).Append(matchingCommandSuggestions[i].command)
					...
					if (suggestedCommand.parameters.Length > 0)
					{
						...
						for (int j = 0; j < suggestedCommand.parameters.Length; j++)
						{
							if (caretParameterIndex != j)
								...
								sharedStringBuilder.Append(commandSuggestionHighlightStart).Append(suggestedCommand.parameters[j])
									.Append(commandSuggestionHighlightEnd);
						}
					}
				...
				{
					commandSuggestionInstances[i].gameObject.SetObjectActive(false);
				}
		...
		private void OnCommandValueChanged( string command )
		{
