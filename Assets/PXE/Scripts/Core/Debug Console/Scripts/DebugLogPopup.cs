...
{
	public class DebugLogPopup : ObjectController, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		public Vector2 defaultPopupPosition = new ( -0.5f, 0.5f );
...
#pragma warning restore 0649
		...
		public override void Awake()
		{
			...
			if( pos.x != 0f || pos.y != 0f )
				...
				normalizedPosition = defaultPopupPosition; // Right edge by default
		}
		...
		public void SetLogCount( int infoCount, int warningCount, int errorCount )
		{
			if (newInfoCount == infoCount && newWarningCount == warningCount && newErrorCount == errorCount)
			{
				return;
			}

			newInfoCount = infoCount;
		...
		private IEnumerator MoveToPosAnimation( Vector2 targetPos )
		{
			...
			while( modifier < 1f )
			{
				modifier += 4f * UnityEngine.Time.unscaledDeltaTime;
